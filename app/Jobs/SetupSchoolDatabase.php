...
final class SetupSchoolDatabase implements ShouldQueue
{
    ...
    public function handle(
        ...
        try {
            ...
            if (!$school) {
                Log::warning("School not found for job ID {$this->schoolId}");
                return;
            }
            ...
            if ($this->packageId) {
                ...
                $cache->removta3limxCache(config('constants.CACHE.SCHOOL.SETTINGS'), $school->id);
            }
            ...
            Mail::send('schools.email', $data, static function ($message) use ($data, $settings) {
                $message->to($data['email'])->from($settings['mail_username'] ?? 'ta3limx Saas')->subject($data['subject']);
            });
